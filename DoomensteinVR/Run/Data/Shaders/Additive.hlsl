...
Texture2D <float4> tBloomBlur : register(t8);
...
{
    float3 hdr_color = tDiffuse.Sample( sSampler, input.uv ).xyz;
    float3 bloom_color = tBloomBlur.Sample( sSampler, input.uv ).xyz;
    hdr_color += bloom_color;   // additive blending

    //float3 result = float3(1,1,1) - exp( -hdr_color * 1.0f );
