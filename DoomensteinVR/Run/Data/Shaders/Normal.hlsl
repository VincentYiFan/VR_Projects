...
v2f_t VertexFunction( vs_input_t input )
{
   ...
   v2f.normal = mul( MATRIX, float4( input.normal, 0 ) ).xyz;
   //v2f.normal = mul( float4( input.normal, 0 ), MATRIX ).xyz;
...
float4 FragmentFunction( v2f_t input ) : SV_Target0
{
    ...
    float3 rgb_normal = normal * 0.5f + 0.5f;
    return float4( rgb_normal, input.color.w );
}