...
void AppendGlyphTriangles2D( std::vector<Vertex_PCU>& verts, char glyph, const Vec2& cellMins, const Vec2& pixelSize, const Rgba8& color )
{
	...
	for( int rowIndex = 0; rowIndex < TRITEXT_PIX_HIGH; ++ rowIndex )
	{
		...
		for( int triTextCellIndex = 0; triTextCellIndex < TRITEXT_PIX_WIDE; ++ triTextCellIndex )
		{
			...
			AppendAABB2D( verts, AABB2( minX,minY, maxX,maxY ), color );
		}
	}
};
...
{
	verts.push_back( Vertex_PCU( Vec3( aabb2.maxs.x, aabb2.mins.y, 0.f ), color, Vec2() ) );
	verts.push_back( Vertex_PCU( Vec3( aabb2.mins.x, aabb2.maxs.y, 0.f ), color, Vec2() ) );
	verts.push_back( Vertex_PCU( Vec3( aabb2.mins.x, aabb2.mins.y, 0.f ), color, Vec2() ) );

	verts.push_back( Vertex_PCU( Vec3( aabb2.maxs.x, aabb2.mins.y, 0.f ), color, Vec2() ) );
	verts.push_back( Vertex_PCU( Vec3( aabb2.maxs.x, aabb2.maxs.y, 0.f ), color, Vec2() ) );
	verts.push_back( Vertex_PCU( Vec3( aabb2.mins.x, aabb2.maxs.y, 0.f ), color, Vec2() ) );
}
